...
class AFMetaModel
{
     ...
     * @var type
     */
    public $fields = ['id', 'name', 'des_name', 'update_period', 'value', 'next_turn', 'time'];
     ...
     * @var type
     */
    public $required_fields = ['name', 'des_name'];
    ...
    public function update($fields = [])
    {
        ...
        if (!$message) {
            ...
            if (!$message) {
                $wpdb->update(
                    $this->table_name, ['name' => $this->name,
                    'des_name'                 => $this->des_name,
                    'value'                    => $this->value,
                    'update_period'            => $this->update_period,
                    'time'                     => $this->time, ], ['id' => $this->id], ['%s', '%s', '%s', '%s']
                );
     ...
     * @return bool
     */
    public function read_if_exists($conditions = [])
    {
     ...
     * @return type
     */
    public function read($count = 'all', $conditions = [], $limit = false, $offset = false)
    {
        if (!empty($fields)) {
            $this->field_values = array_merge($this->field_values, $fields);
        }
