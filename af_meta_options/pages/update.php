...
function af_meta_update_view($message, $fields)
{
    ...
    <div class="wrap">
        <h2>
            Update Meta Option
            <a class="add-new-h2" href="<?php echo admin_url('admin.php?page=update_meta_options&id=' . $fields['id'] . '&action=delete'); ?>" onclick="if (!window.confirm('are you sure?'))
                        return false">Delete Item</a>
        ...
        <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
            <table class='wp-list-table widefat fixed'>
                ...
                <tr>
                    <th>Time</th>
                    <td>
                        <input type="text" class="af_time" name="time" value="<?php echo date('Y-m-d',  strtotime($fields['time'])); ?>" />
